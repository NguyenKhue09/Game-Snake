...
void abcd()
{
	...
	SetTeColor(14);
	char* _=new char[40];
	strcpy(_,"%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c");
	int __ = 3 * *_ + 5;
	printf("\n\n    \t\t\t");
...
int DieuKhien(int n, int x, int y)
{
		...
		if (_kbhit())
		{
			...
			if (a == 112)
			{
				...
				while (1)
				{
					...
					gotoxy(10, 21);
					Sleep(100);
					printf("Dang tam dung bam P de tiep tuc -- Bam ESC de qoay lai MENU");
					gotoxy(10, 21);
					Sleep(100);
					cout<<"                                                           ";
				}
	...
	if (Check() == false)
	{
		...
		SetTeColor(12);
		cout<<"\t\tGame Over"<<endl;
		cout<<"\tDiem cua ban la : "<< DiemCT + DiemV<<endl;
		char tmp;
		SetTeColor(15);
		cout<<"\tBam ENTER de bat dau game moi\n\tBam ESC de thoat Game";
		while (1)
		{
			tmp = _getch();
			if (tmp == 13)
...
void xuatDiem()
{
	...
	gotoxy(1, 23);
    cout<<"DIEM CUA BAN LA: "<<DiemCT + DiemV;
}
void xoaTail(ToaDo cuoi)
{
...
bool Check()
{
	if (Head.x == 1 || Head.x == 79 || Head.y == 1 || Head.y == 20)
		return false;
	for (int i = 0; i < DoDai - 1; i++)
...
void menu()
{
	...
	if (chon == 2)
	{
		...
		cout << "Bam enter de choi game" << endl;
	...
	if (chon == 3)
	{
		...
		cout<<"Khi an duoc 5 con moi thi se xuat hien con moi dac biet\n\tton tai tron 90 buoc di cua con ran"<<endl;
		cout<<"Khi an con moi dac biet se duoc diem rat la cao"<<endl;
		cout<<"cung kha don gian"<<endl;
		cout<<"Bam enter de qua lai luc nay__"<<endl;
		char enter;
		while (1)
		{
			enter = _getch();
